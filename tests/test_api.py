...
class ApiTests(unittest.TestCase):
    ...
    def test_login_redirects_to_career_galaxy(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["data"]["redirect"], "/select-track")
    ...
    def test_world_chat_persists_message_and_main_profile_identity(self) -> None:
        ...
        self.assertTrue(sent_message["sender"]["profileImage"].startswith(
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    (
                        buddy_uid,
                        buddy_email,
                        bcrypt.hashpw(b"buddy-pass-1234", bcrypt.gensalt()).decode("utf-8"),
                    ),
                )
